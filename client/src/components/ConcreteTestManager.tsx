                    ...
                    <tbody className="divide-y divide-concrete-100 bg-white">
                      {formData.specimens.map((s, idx) => (
                        <tr key={idx} onClick={() => handleSpecimenClick(idx)} className="hover:bg-blue-50 cursor-pointer transition-colors">
                          ...
                          <td className="px-3 py-2 text-right"><input type="number" placeholder="+ Saisir les données" className="w-20 p-1 text-right border-transparent hover:border-concrete-300 bg-transparent" onClick={e=>e.stopPropagation()} value={s.weight ?? ''} onChange={e=>handleInlineChange(idx,'weight',e.target.value)}/></td>
                          <td className="px-3 py-2 text-right"><input type="number" placeholder="+ Saisir les données" className="w-20 p-1 text-right border-transparent hover:border-concrete-300 bg-transparent" onClick={e=>e.stopPropagation()} value={s.force ?? ''} onChange={e=>handleInlineChange(idx,'force',e.target.value)}/></td>
